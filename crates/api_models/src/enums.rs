...
// --- REMITTANCES INTEGRATION --- (feature = "remittances")
#[cfg(feature = "remittances")]
pub mod remittances_integration {
    use super::Connector;
    use serde::{Deserialize, Serialize};
    use strum::{Display, EnumString};
    use utoipa::ToSchema;

    /// Conectores que soportan el flujo de remesas
    #[derive(
    ...
    #[serde(rename_all = "snake_case")]
    #[strum(serialize_all = "snake_case")]
    pub enum RemittanceConnectors {

        Paypal,
        Ebanx,
    }

    impl From<RemittanceConnectors> for Connector {
        fn from(c: RemittanceConnectors) -> Self {
            match c {

                RemittanceConnectors::Paypal => Connector::Paypal,
                RemittanceConnectors::Ebanx  => Connector::Ebanx,
            }
        }
    }

    impl TryFrom<Connector> for RemittanceConnectors {
        type Error = String;

        fn try_from(conn: Connector) -> Result<Self, Self::Error> {
            match conn {
                Connector::Paypal => Ok(RemittanceConnectors::Paypal),
                Connector::Ebanx  => Ok(RemittanceConnectors::Ebanx),
                other => Err(format!("{:?} no soporta remesas", other)),
            }
        }
    }

    /// Canal de envío / recepción
    #[derive(
    ...
    /// Tipo de entidad remitente/beneficiario
    #[derive(
        Clone, Copy, Debug, Eq, PartialEq,
        Serialize, Deserialize,
        Display, EnumString,
    ...
    #[serde(rename_all = "snake_case")]
    #[strum(serialize_all = "snake_case")]
    pub enum RemittanceSendPriority {
        Immediate,
        Scheduled,
    }
}
...
#[serde(rename_all = "snake_case")]
pub enum RemittanceMethod {
    /// Envío a cuenta bancaria (ACH, SEPA, etc.)
    BankTransfer,
    /// Retiro en ventanilla / corresponsal
    CashPickup,
    /// Abono a monedero electrónico
    Wallet,
    /// Abono directo a tarjeta
    Card,
}
