...
#[cfg(feature = "remittances")]use crate::routes::remittances::*;
...
pub struct Remittances;

// Implementación para /remittances (v1)
#[cfg(all(feature = "remittances", feature = "v1", feature = "oltp"))]
impl Remittances {
    pub fn server(state: AppState) -> Scope {
        web::scope("/remittances")
            ...
            .service(
                web::resource("")
                    .route(web::post().to(remittances_create))
                    .route(web::get().to(remittances_list)),
            )
            .service(
                web::resource("/quote")
                    .route(web::post().to(remittances_quote)),
            )
            .service(
                web::resource("/sync")
                    .route(web::post().to(remittances_sync_batch)),
            )
            .service(
                web::scope("/{remittance_id}")
                    .service(
                        web::resource("")
                            .route(web::get().to(remittances_retrieve))
                            .route(web::patch().to(remittances_update))
                            .route(web::delete().to(remittances_cancel)),
                    )
                    .service(
                        web::resource("/pay")
                            .route(web::post().to(remittances_pay)),
                    )
                    .service(
                        web::resource("/sync")
                            .route(web::post().to(remittances_sync)),
                    ),
            )
            .service(
                web::resource("/webhooks/{merchant_id}/remittances/{connector}")
                    .route(web::post().to(remittances_webhook)),
            )
    }
}

// Implementación para /v2/remittances (v2)
#[cfg(all(feature = "remittances", feature = "v2", feature = "oltp"))]
impl Remittances {
    pub fn server_v2(state: AppState) -> Scope {
        web::scope("/v2/remittances")
            .app_data(web::Data::new(state))
            .service(
                web::resource("")
                    .route(web::post().to(remittances_create))
